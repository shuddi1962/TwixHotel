...
export function RoomCard({ room }: { room: RoomCardData }) {
  const image = room.images?.[0] || `https://picsum.photos/seed/room-${room.id}/800/900`

  return (
    <div className="room-card">
      <div className="room-card__image">
        <Image src={image} alt={room.room_type} fill sizes="(max-width: 768px) 100vw, 33vw" />
        <div className="absolute top-3 right-3 rounded-full px-3 py-1 text-xs font-medium backdrop-blur-sm" style={{ background: "rgba(0,0,0,0.5)", color: "var(--vg-gold-light)" }}>
          {room.currencySymbol}{room.price_per_night.toFixed(0)} / night
        </div>
      </div>
      <div className="room-card__body">
        <p className="text-xs uppercase tracking-widest text-white/30 mb-1">Room {room.room_number}</p>
